...
BigInt::BigInt(const BigInt &&n) : size(n.size), is_negative(n.is_negative), data(n.data)
{
  n.data = nullptr;
  n.size = 0;
}
...
BigInt BigInt::plus(const BigInt &n1, const BigInt &n2) const
{
  ...
  for (size_t i=n1.size, j=n2.size; i >= 0 && j >= 0; --i, --j)
  {
    res[current] = (n1.data[i] + n2.data[j] + left) % 10;
    left = (n1.data[i] + n2.data[j] + left) / 10;
    current--;
  }
  for (size_t i=n2.size-n1.size; i >= 0; --i)
  {
